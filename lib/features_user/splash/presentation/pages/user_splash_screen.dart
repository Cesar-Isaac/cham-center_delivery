...
/// شاشة انطلاق نسخة المستخدم — بنفس تصميم شاشة انطلاق السائق.
class UserSplashScreen extends StatefulWidget {
  const UserSplashScreen({super.key});

  static const String route = '/user-splash';
...
class _UserSplashScreenState extends State<UserSplashScreen>
  ...
  // ── Controllers ────────────────────────────────────────────────────────────
  late final AnimationController _bagCtrl;    // bag slide-in
  ...
  // ── Animations ─────────────────────────────────────────────────────────────
  late final Animation<double> _bagX;
  late final Animation<double> _bagScale;
  ...
  void initState() {
    ...
    // Bag slides in from the left, bounces slightly
    _bagX = Tween<double>(begin: -1.6, end: 0).animate(
      CurvedAnimation(parent: _bagCtrl, curve: Curves.easeOutCubic),
    );
    _bagScale = Tween<double>(begin: 0.6, end: 1.0).animate(
      CurvedAnimation(parent: _bagCtrl, curve: Curves.easeOutBack),
    );
          ...
          Center(
            child: Column(
              ...
              children: [
                // Shopping bag icon
                AnimatedBuilder(
                  animation: Listenable.merge([_bagCtrl, _pulseCtrl]),
                  builder: (_, _) => FractionalTranslation(
                    translation: Offset(_bagX.value, 0),
